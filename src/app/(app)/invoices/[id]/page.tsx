      ...
      >
        {canManage && (
          <>
            ...
            <SagaXmlDownloadButton invoiceId={invoice.id} needsPersonalization={invoice.needsPersonalization} />
            <InvoiceFormDialog
              invoice={formData}
            ...
            <CardContent className="space-y-3 text-sm">
              <Row label="Status">
                <div className="flex flex-wrap items-center justify-end gap-1">
                  <Badge variant={invoiceStatusVariant(invoice.status)}>{INVOICE_STATUS_LABELS[invoice.status]}</Badge>
        ...
        <div className="space-y-6 md:col-span-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <Detail label="Invoice generated by">
                {invoice.selfIssued ? (
                  <Badge variant="outline">Us — self-issued</Badge>
                ) : (
                  <span className="text-muted-foreground">Accounting firm</span>
                )}
              </Detail>
              {invoice.selfIssued && (
                <Detail label="Number series">{invoice.series?.prefix ?? "Default series"}</Detail>
              )}
              <Detail label="Needs monthly personalization">
                {invoice.needsPersonalization ? (
                  <Badge className="gap-1 border-transparent bg-violet-500/15 text-violet-600 dark:text-violet-400">
